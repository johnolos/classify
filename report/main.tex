...
The meaning of a name entity regognition system is to classify words into different entitys we choose to classify on the entitys Organization, Location, Time, Person and Other. To solve this problem we approached the problem using the viterbi algorithm, which uses a hidden markov model to find the given entites probability based on the previous entities in sentene. With the name enitity recognition system we aim to answer which kind of entity each word is, entities we want to find is Other, Location, Person, Time and Orgainization.

\section{Introduction}
At the beginning of the project we looked into making the name entity recognition system using a sequence model, which the neme entity recognition system made at Stanford is using we wanted to use this because after some reading we thought it would give us the best final result. But after reading some papers we did not find any good description on how to implement it. Then after some consideration and searching into other ways to do name entity recognition we found the viterbi algorithm and decided to implement it in java, the viterbi algorithm uses hidden markov model to find the highest probability of a words entity given by the previous entities.

\section{Background}
To make the name entity recognition system we ended up using the viterbi algorithm and hidden markov model, since none of us have had anything to do with hiddem markov models earlier we where not quite shure how to implement it. To start the project we started start probability, emission probability and a transition matrix which probabilities based on the training data set. By using these probabilities we can use the hidden markov model to find the highest probability of a word being a any of the five entities we aim to classify. Our goal is to get the algorithm working before we start to find or make training sets which is of decent size, this is because we think the hardest part of the project is to make the hidden markov model work. To check if the algorithm is working we will only be using a small training set which we have created, the downsize of using a small training data set is that you will get terrible results. It is important to remember to have training data sets be built up by sentences because it will help the algorithm to classify based on the "flow" in the entities. Our hope is to get a success probability around 80\% for our name entity recognition system.


\section{Body of report}
At the start of the project we looked closer at doing a sequence model name entity recognition system, but after reading some papers and turning up empty in the search for an algorithm to follow. Then we choose to chase down the hidden markov model approach to name entity recognition, after reading paper reading we ended up using the viterbi algorithm.

\subsection{Training Data Set}
To start the project we first made the file to read the words and their entity from the training data set(s), the training data set(s) word sequence needs to be built up like real sentences because this will improve the entity prediciton in the program. The words will also need to have their entity written after it with a space between them. This means that the data in the text files can be written in two different ways. Either you can have one word and its entity in one line, or you can have multiple words and their entity in one line see figure \ref{fig:trainingData}. Our training data set looks like the dataset you see on the right hand side of the figure, it also contains 2518 classified words which we have classified manually. This means that some words in the training data may have one or more entity like New which is a location when it is in context with New York or it may be other when it is just by itself.
...
We have made a enum java class which holds the different entities the program is to classify, and in the file which reads in the training data set we use both hashmap and arraylists to keep track of the words and their entity, the reason for making so many lists is that they make it easier to calculate the different probabilities used by the hidden markov model even if they hold redundant information.
...
\end{minted}
We found this library most useful because we needed a good way to store a probability which is decided by two variables, and it is used in both the emission, transmission probability matrix and the hidden markov model to store the updated values for each step.
...
The emission probability matrix is the probability of a specific word being a specific entity. If you look at table \ref{tab:emissionProbability} and the word Obama it has the probability of $\frac{1}{15}$ for person and zero for every other entity, this gives us that the word Obama is most likely a person.
\begin{equation}
...
\end{equation}
The transmission probability matrix is in our case 5x5(number of entities*number of entities), this is because it shows the probability of the next word in a sentence given the current words entity.
...
Since our implementation of the hidden markov model identifies entities based on sentences the first word in every sentence is a special case. Note that no of the probability’s can be 0 it should be 0.0000000001 instead, because when you take the logarithm of 0 you will get negative infinity and the algorithm will give you wrong classifications.
\begin{equation}
...
\end{equation}
The words entity is given by whcih entity is given the highest probability by the hidden markov model or the entity which has the lowest negative number in our case. To get the entities for the rest of the words in the sentence, we need to use the earlier words in the sentence entities. Based on the previously entities we need to find the entity path with the highest overall probability, to get the next words entity you use this equation:
\begin{equation}
\begin{split}
...
By using regex expressions we have tried to help the system identify the correct entity for the different words, we use the regex expressions to help us when the viterbi algorithm gives a word enity other. Because when the algorithm gives the enitity other it is because it is the entity which most often occur in texts. Then hopefully the regex expressions we have made can help us to identify more words with the right entity. We have made regex expressions to help us identify time, persons and Location entities. 
...
\end{table}
By looking at the correct and Error rate we can see that all of our articles got a better result than we hoped when we started, so we are happy with how the system runs and gives out results.
